...
| Species images | Used exactly as the original app uses them; silhouettes derived from the same images | `SAM-04` |
...
| SAM-04 | Undetected species as **silhouettes**, detected ones with a photo | **P0** | The Pokédex effect in a single design decision. **Decided: species images are used exactly as the original app uses them** — the bundle pipeline already downloads and packs a 480×320 WebP per species, so there is no procurement project and no new licensing question to settle. **The silhouettes should be derived from those same images** in the same pipeline (desaturate, flatten to a single dark tone, keep the outline) rather than drawn by hand: that covers all ~9,800 species instead of a curated 130, stays correct whenever the taxonomy is rebuilt, and gives every species a consistent "not yet found" state. Attribution has to follow the image into the derivative |
...
4. **The collection** — SAM-02 to SAM-05, AUS-01/02/03, STAT-01/02/05/06
