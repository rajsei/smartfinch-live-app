...
| **D6** | Species images | **Use them exactly as the original app does** | The existing bundle pipeline and its licence situation carry over unchanged. No new licensing project. Silhouettes for `SAM-04` are then best derived from the same images rather than drawn from scratch — see §4 |
...
>
> What remains is `SAM-04`'s **silhouettes**. Do not commission 130 drawings — derive them from the images that are already there. A silhouette pass in the same pipeline (desaturate, flatten to a single dark tone against the surface colour, keep the outline) gives every species a consistent "not yet found" state for free, scales to all ~9,800 species rather than a curated 130, and stays correct when the taxonomy is rebuilt. Attribution must follow the image into the silhouette, since it is a derivative.
>
...
**0.3 · Delete the research modes.** `features/survey`, `features/aru`, `features/point_count`, `features/file_analysis` — ~20,000 lines. Only three files import them (`lib/app.dart`, `lib/main.dart`, `shared/services/quick_action_service.dart`). Also: `widgets/voice_memo_overlay.dart`, `session_map_screen.dart`, `shared/services/shared_media_service.dart`, the expert inference controls, and the Raven/GPX export writers.

**0.4 · Collapse what the deletion exposed.** `SessionType` drops from six values to one; `shared/utils/session_type_visuals.dart` goes. `SessionSettings` sheds ~30 survey/ARU fields — **keep them tolerated on read** so existing files still deserialise, drop them on write. Prune the twelve files in `features/history` that branch on session type.
...
**2.6 · The Collection** (`SAM-02`…`SAM-05`, `SAM-17`) as its own area: found species only, all taxon groups by default with a group filter, silhouettes for the open ones, progress counter. **Needs the silhouette pass in the bundle pipeline** — start that early, it is asset work, not code.
...
4. **Silhouette rendering** (§4) — needs one pass in the bundle pipeline, and a decision on how attribution follows a derivative image.
5. **Windows build** — still exists, still costs CI time, not in the specification's target platforms. Decide explicitly: drop it, or keep it unmaintained behind a note.
6. **`SET-12` defaults need a real number.** The deletion rule (most-recorded species, lowest confidence first, 30 days, 100 clips per species) is sound, but nobody has measured what a child actually accumulates in a month. Set generous defaults, then check against the field test rather than guessing now.
7. **Do favourites survive the cap?** Proposed in D10 with a question mark. Recommendation: yes, and show the count — "12 of your 100 favourites" — so a child understands why the number stops growing. It is cheap and it prevents the one loss that would actually hurt.
